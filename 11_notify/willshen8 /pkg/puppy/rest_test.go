...
func TestNotifyLostPuppyWith201Response(t *testing.T) {
	http.DefaultClient = goodResponseClient()
	rr := httptest.NewRecorder()
	notifyLostPuppy(1, rr)
}

func TestNotifyLostPuppyWith500Response(t *testing.T) {
	http.DefaultClient = badResponseClient()
	rr := httptest.NewRecorder()
	notifyLostPuppy(111, rr)
}

// full credit goes to Daniel Cantos for helping/teaching me mocking api responses
...
func goodResponseClient() *http.Client {
	return &http.Client{
