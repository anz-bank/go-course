...
type MapStore struct {
	puppies map[uint16]puppy.Puppy
}

// NewMapStore creates a new in-memory store with map intialised
func NewMapStore() *MapStore {
	return &MapStore{puppies: map[uint16]puppy.Puppy{}}
}
...
func (m *MapStore) ReadPuppy(id uint16) (puppy.Puppy, error) {
...
func (m *MapStore) UpdatePuppy(id uint16, p *puppy.Puppy) error {
	if err := puppy.ValidateValue(p.Value); err != nil {
		return err
	}
